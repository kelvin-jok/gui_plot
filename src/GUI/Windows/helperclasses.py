    ...
    def __init__(self, chk_lbl, win_title, grp_title, col_title, groupings, rowdata, chan_cols="", plot_cols="", raw_cols="", filter_window=False, filter_lbl=False):
        ...
        if len(chk_lbl) > 0:
            if filter_window:
                ...
                ok_button.clicked.connect(lambda: self.selected(win, groupings, self.colorby_txt.toPlainText(), raw_box.grp_box.findChildren(QCheckBox)[0].isChecked(), self.axis.boxes))
...
#            win.layout().addWidget(col_txt, 1, 1)
 #           win.layout().addWidget(filter_btn, 0, 1)
    ...
    def selected(self, win, groupings, grp_box, raw=False, axes=False):
