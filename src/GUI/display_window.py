...
class displayWindow(QWidget):
    def __init__(self):
        ...
        piemaps.triggered.connect(lambda: piechart(self.plot_data, self.filtered_data, self.numcluster, np.array(self.labels), [np.array(plot.get_facecolor()[0][0:3]) for plot in self.plots]) if len(self.plot_data) > 0 else errorWindow("Error Dialog","Please Select Feature File. No data is currently displayed"))
    ...
    def color_groupings(self, grouping, plot, new_plot):
        ...
        if 'csv' in self.feature_file[0]:
            df = pd.read_csv(self.feature_file[0], nrows=1)
        else:
            df=pd.read_csv(self.feature_file[0], sep='\t', nrows=1)
        chk_lbl=list(df.columns)
        '''
        if np.shape(df.values)[1] == 1:
            chk_lbl = chk_lbl[0].rsplit(",")
            #print(list(map(str.strip, chk_lbl[0].rsplit(','))))
        print(chk_lbl)
        chk_lbl=list(filter(None, chk_lbl))
            #chk_lbl.remove('')
        '''
        print(chk_lbl)
        print("here")
        '''
        print(df)
        ch_lbl = df.apply(lambda path: os.path.exists(path.values[0]))#os.path.exists(str(path)))
        print(ch_lbl)
        ch_lbl=list(ch_lbl.index[ch_lbl==True])
        print(ch_lbl)
        '''
        '''
        #forward slash
        ch_lbl_for=df.stack().str.contains('/').groupby(level=1).any()#.apply(lambda lbl: len(lbl)>1)
        print(ch_lbl_for)
        #ch_lbl_for=list(ch_lbl.index[ch_lbl == True])
        ch_lbl_back=df.stack().str.contains(r"\\").groupby(level=1).any()
        print(ch_lbl_back)
        ch_all=pd.concat([ch_lbl_for, ch_lbl_back], axis=1)
        #ch_all=list(ch_all.index[ch_all == True])
        ch_all=ch_all.any(axis=1)
        ch_all=list(ch_all.index[ch_all==True])
        print(ch_all)
        '''
        num_lbl=list(df._get_numeric_data().columns) #strings so dont work?
        print("num_lbl",num_lbl)
    ...
    def data_filt(self, grouping, projection, plot, new_plot):
        ...
        def id_labels(feature_data, X, plot):
            ...
            # reset imageIDs
            #self.imageIDs.clear()
            #if len(self.ch_path) > 0 or plot !="Raw Data":
            #    self.imageIDs.extend(np.array(feature_data['ImageID'].loc[X.index],dtype='object').astype(int))
            # reset labels
            ...
            if filter_data != "No Grouping":
                ...
                '''
                if str.isdigit(z[0]):
                    self.labels.extend(list(map(int, z)))
                else:
                    dec = len(z[0]) - str(z[0]).find(".")
                    print(str(z[0]).find("."), z[0].find("."))
                    print(len(z[0]), dec)
                    self.labels.extend(list(map(float, z)))
                    lbl = np.round(z, dec)
                '''
            #else:
            #    self.labels.extend(list(map(str, z)))
        ...
        if plot =="Raw Data":
            print(np.where(np.array(self.raw_col)!='None'))
            print(np.array(self.raw_col)[np.where(np.array(self.raw_col)!='None')])
            X=feature_data.copy()
            ...
            #print(feature_data)
            #feature_data.sort_values(list(featurecols), axis=0, inplace=True)
            #print(feature_data)
