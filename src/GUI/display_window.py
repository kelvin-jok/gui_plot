...
class displayWindow(QWidget):
    def __init__(self):
        ...
        piemaps = clustering.addAction("Cluster Pie Maps (2D - X and Y axis display)")
        ...
        def check_projection(dim, plot):
            if dim == "2d":
                ...
                self.main_plot.draw()
                #rotate left click, zoom right click (disabled) for key mapping see https://matplotlib.org/stable/api/backend_bases_api.html#matplotlib.backend_bases.MouseButton
    ...
    def color_groupings(self, grouping, plot, new_plot, prevfile):
        #read feature file
        ...
        grp=[]
        if grouping.count()>1 and isinstance(prevfile, str):
            grp=[grouping.itemText(i + 1) for i in range(grouping.count() - 1)]

        #select features window
        win=selectWindow(chk_lbl, "Column Selection - Feature File", "Grouping", "Channels", grp, df, (lambda prev: self.ch_path if isinstance(prev, str) else [])(prevfile),
                         (lambda prev: self.plotcols if isinstance(prev, str) else [])(prevfile), (lambda prev: self.raw_col if isinstance(prev, str) else [])(prevfile), True, num_lbl)
        if not win.x_press:
    ...
    def data_filt(self, grouping, projection, plot, new_plot):
        try:
            ...
            #filter and get labels
            def id_labels(feature_data, X, plot):
                print('Dataset shape:', feature_data.shape)
                if plot=='Raw Data':
                    feature_data.dropna(subset=list(filter(lambda x: x!='None', self.raw_col)), axis=0, inplace=True)
            ...
            if plot =="Raw Data":
                ...
                if np.array_equal(mind, maxd) == False:
                    ...
                    errorWindow('Feature File Data Error', 'Check if have more than 1 row of data and that min and max values of columns are not the same')
        except:
