    ...
    def __init__(self, plot_data, datafilt, numclusters, featurefile):
        if numclusters != None:
            ...
            if name[0] != '':
                ...
                cols = list(
                    filter(lambda col: (col[:2] != 'MV' and col != 'bounds' and col != 'intensity_thresholds'), cols))
                ...
                data = data[cols]
                cols = list(
                    pd.read_csv(data["MetadataFile"].str.replace(r'\\', '/', regex=True).iloc[0], nrows=1, sep='\t'))
                if 'Stack' in cols:
                    stack = []
                    metadata = pd.read_csv(data["MetadataFile"].str.replace(r'\\', '/', regex=True).iloc[0],
